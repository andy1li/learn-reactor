...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor().concatMap(connection -> connection);
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary()
                .switchOnFirst((first, mailBoxPrimary) ->
                        first.get().metaData.contains("spam")
                            ? mailBoxSecondary()
                            : mailBoxPrimary
                );
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds =
                tasksToExecute().concatMap(
                    task -> task.flatMap(
                        taskId -> commitTask(taskId).thenReturn(taskId)
                    )
                );
